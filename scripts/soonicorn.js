...
const {soonicorn} = require('../schemas/schemas')

exports.get_all_soonicorn = async() =>{
    try{
        let start_from = 0;
        let size = 100
        while(true){
            var data = JSON.stringify({"dataset":"query","sort":[{"sortField":"soonicornEventDate","order":"DEFAULT"}],"query":{},"filter":{"and":[{"soonicornFlag":["SOONICORN"]}]},"view":"card","size":size,"from":start_from});
            ...
            await soonicorn.insertMany(results.data.result)
            console.log('ALL soonicorn PAGE: ' + start_from)
